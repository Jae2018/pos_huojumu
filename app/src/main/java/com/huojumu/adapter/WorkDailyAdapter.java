 ...
 */
public class WorkDailyAdapter extends BaseQuickAdapter<OrderNoList.RowsBean, BaseViewHolder> {

    public WorkDailyAdapter(@Nullable List<OrderNoList.RowsBean> data) {
        super(R.layout.item_wor_daily_takeover, data);
    }

    @Override
    protected void convert(BaseViewHolder helper, OrderNoList.RowsBean item) {
        helper.setText(R.id.tv_daily_list_item_no, String.format("订单编号：%s", item.getOrdId()))
                .setText(R.id.tv_daily_list_item_time, String.format("生成时间：%s", item.getCreateTime()))
                .setText(R.id.tv_daily_list_item_money, String.format("订单总价：%.2s", item.getTotalPrice()));
    }
